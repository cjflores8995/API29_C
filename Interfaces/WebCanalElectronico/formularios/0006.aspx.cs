...
public partial class formularios_0006 : System.Web.UI.Page
{
    ...
    protected void btnProcesar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (txtfcontable.Text != "")
            {
                if (Util.ValidaFechas(txtfcontable.Text))
                {
                    btnProcesar.Disabled = true;
                    TSISUSUARIO objUsuario = (TSISUSUARIO)Session["sesionUsuario"];
                    batch.CargaDesbloqueos(txtfcontable.Text, objUsuario.CUSUARIO, out proceso, out error, out registrosCorrectos, out registrosError);
                    if (error == "OK")
                    {
