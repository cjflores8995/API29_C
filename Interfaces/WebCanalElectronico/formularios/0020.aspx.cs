...
public partial class formularios_0020 : System.Web.UI.Page
{
    ...
    protected void btnProcesar_ServerClick(object sender, EventArgs e)
    {
        ...
        try
        {
            if (txtFechaProceso.Text != "")
            {
                if (Util.ValidaFechas(txtFechaProceso.Text))
                {
                    ...
                    batch.ProcesaConvivenciaSifco(txtFechaProceso.Text, objUsuario.CUSUARIO, out proceso, out error, out registrosCorrectos, out registrosError);
                    if (error == "OK")
                    {
