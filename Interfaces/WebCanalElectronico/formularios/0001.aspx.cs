...
public partial class formularios_0001 : System.Web.UI.Page
{
    ...
    protected void btnProcesar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (txtFechaCorte.Text != "")
            {
                ...
                path = ConfigurationManager.AppSettings["pathArchivos"].Trim() + string.Format(ConfigurationManager.AppSettings["pathArchivosEstructuras"].Trim(), "UAF", DateTime.Now.ToString("yyyyMMddHHmmss"));
                if (!Directory.Exists(path))
                    Directory.CreateDirectory(path);
                Thread.Sleep(5000);
                respuesta = est.GeneraUAF(Convert.ToDateTime(txtFechaCorte.Text), path, out nombreZip);
                ...
                if (respuesta.CError == "000")
                {
                    ...
                    ScriptManager.RegisterStartupScript(this.panelformulario, GetType(), "alerta", Util.MostarAlertaFormularios("", "ESTRUCTURA GENERADA CORRECTAMENTE\\nTIEMPO: " + tiempo, "OK"), true);
                }
                else
                {
